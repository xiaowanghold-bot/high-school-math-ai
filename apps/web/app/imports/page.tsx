...
type CurriculumSuggestion = { node_id: string; name: string; volume: string; chapter: string; section: string; confidence: number; reasons: string[] };
type CurriculumMapping = { volume: string | null; chapter: string | null; section: string | null; knowledge_point_ids: string[]; knowledge_point_names: string[] };
type CurriculumBatchItem = { question_id: string; stem_plain: string; source_document: string; source_page_start: number | null; source_page_end: number | null; current_curriculum: CurriculumMapping; suggestions: CurriculumSuggestion[]; recommendation_status: "already_mapped" | "high_confidence" | "review_required" | "no_suggestion" };
type CurriculumBatch = { total: number; mapped_count: number; high_confidence_count: number; review_required_count: number; no_suggestion_count: number; items: CurriculumBatchItem[] };
...
function ImportsPageContent() {
  ...
  const [viewMode, setViewMode] = useState<"pages" | "boundaries" | "structured" | "curriculum">("pages");
  ...
  const [curriculumBatch, setCurriculumBatch] = useState<CurriculumBatch | null>(null);
  const [curriculumSelections, setCurriculumSelections] = useState<Record<string, string>>({});
  ...
  const activeCurriculumItem = useMemo(() => curriculumBatch?.items.find((item) => item.question_id === draft?.imported_question_id) ?? curriculumBatch?.items[0] ?? null, [curriculumBatch, draft]);
  ...
  async function loadCurriculumBatch(sourceDrafts: StructuredDraftList = drafts) {
    const questionIds = sourceDrafts.items.flatMap((item) => item.imported_question_id ? [item.imported_question_id] : []);
    if (!questionIds.length) { setCurriculumBatch(null); setCurriculumSelections({}); setSelectedCurriculumIds([]); return; }
    const response = await fetch("/api/v1/questions/quality/curriculum/batch/inspect", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ question_ids: questionIds }),
    });
    if (!response.ok) throw new Error(await errorText(response));
    const payload: CurriculumBatch = await response.json();
    setCurriculumBatch(payload);
    setCurriculumSelections(Object.fromEntries(payload.items.flatMap((item) => item.suggestions[0] ? [[item.question_id, item.suggestions[0].node_id]] : [])));
    setSelectedCurriculumIds(payload.items.filter((item) => item.recommendation_status === "high_confidence").map((item) => item.question_id));
  }
  ...
  async function openFile(fileId: string) {
    ...
    setSelected(detail);
    setPreviewPage(1);
    const draftResponse = await fetch(`/api/v1/imports/files/${fileId}/structured-drafts`);
    if (!draftResponse.ok) throw new Error(await errorText(draftResponse));
    const draftPayload: StructuredDraftList = await draftResponse.json();
    setDrafts(draftPayload);
    const nextDraft = draftPayload.items[0] ?? null;
    setDraft(nextDraft);
    await Promise.all([loadBoundaries(fileId), loadSourcePairing(fileId), loadCurriculumBatch(draftPayload)]);
  }
  ...
  async function importDraft() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(await errorText(response));
      const result = await response.json();
      const draftResponse = await fetch(`/api/v1/imports/files/${selected.file_id}/structured-drafts`);
      if (!draftResponse.ok) throw new Error(await errorText(draftResponse));
      const updatedDrafts: StructuredDraftList = await draftResponse.json();
      setDrafts(updatedDrafts); setDraft(updatedDrafts.items.find((item) => item.draft_id === result.draft.draft_id) ?? null);
      await loadCurriculumBatch(updatedDrafts);
  ...
  function selectCurriculumItem(item: CurriculumBatchItem) {
    const sourceDraft = drafts.items.find((candidate) => candidate.imported_question_id === item.question_id);
    if (sourceDraft) { setDraft(sourceDraft); setPreviewPage(sourceDraft.start_page); }
  }

  function toggleCurriculumSelection(questionId: string) {
    setSelectedCurriculumIds((current) => current.includes(questionId) ? current.filter((item) => item !== questionId) : [...current, questionId]);
  }

  async function applyCurriculumMappings(questionIds: string[]) {
    const mappings = questionIds.flatMap((questionId) => curriculumSelections[questionId] ? [{ question_id: questionId, node_id: curriculumSelections[questionId] }] : []);
    if (!mappings.length) { setMessage("请先选择至少一道题和一个具体知识点。"); return; }
    setBusy(true);
    try {
      const response = await fetch("/api/v1/questions/quality/curriculum/batch/apply", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ mappings, teacher_id: "owner_teacher" }),
      });
      if (!response.ok) throw new Error(await errorText(response));
      const result = await response.json();
      await loadCurriculumBatch();
      setMessage(result.message);
    } catch (error) { setMessage(error instanceof Error ? error.message : "教材映射保存失败"); }
    finally { setBusy(false); }
  }
  ...
  return <div className="page-content import-workspace">
    ...
    <ResizableColumns className="import-layout" storageKey="pdf-import-queue" initialLeftPercent={32} leftMin={260} rightMin={480} collapse="compact" label="调整 PDF 处理队列与加工区宽度">
      ...
      <main className="import-inspector">{!selected ? <div className="import-inspector-empty"><span>PDF</span><h2>选择文件检查页面质量</h2><p>这里会显示原文件、文字层覆盖、题号标记和需要 OCR 的页面。</p></div> : <>
        ...
        <nav className="import-stage-tabs"><button type="button" className={viewMode === "pages" ? "active" : ""} onClick={() => setViewMode("pages")}><span>01</span><div><strong>逐页分析</strong><small>文字层、题号与图片</small></div></button><button type="button" className={viewMode === "boundaries" ? "active" : ""} disabled={selected.status !== "ready_for_segmentation"} onClick={() => setViewMode("boundaries")}><span>02</span><div><strong>题目边界</strong><small>{boundaries.total ? `${boundaries.confirmed_count}/${boundaries.total} 已确认` : "生成候选后人工校对"}</small></div></button><button type="button" className={viewMode === "structured" ? "active" : ""} disabled={!boundaries.confirmed_count} onClick={() => setViewMode("structured")}><span>03</span><div><strong>内容结构化</strong><small>{drafts.total ? `${drafts.confirmed_count + drafts.imported_count}/${drafts.total} 已校对` : "题干、选项、公式与配图"}</small></div></button><button type="button" className={viewMode === "curriculum" ? "active" : ""} disabled={!drafts.imported_count} onClick={() => { setViewMode("curriculum"); loadCurriculumBatch().catch((error: Error) => setMessage(error.message)); }}><span>04</span><div><strong>教材映射</strong><small>{curriculumBatch ? `${curriculumBatch.mapped_count}/${curriculumBatch.total} 已确认` : "人教 A 版批量标注"}</small></div></button></nav>
        <ResizableColumns className={`import-preview-layout ${viewMode === "boundaries" ? "boundary-mode" : viewMode === "structured" ? "structured-mode" : viewMode === "curriculum" ? "curriculum-mode" : ""}`} storageKey={`pdf-preview-${viewMode}`} initialLeftPercent={viewMode === "pages" ? 63 : 45} leftMin={viewMode === "pages" ? 340 : 320} rightMin={viewMode === "pages" ? 240 : 360} collapse="wide" label="调整 PDF 原文预览与分析校对区宽度">
          ...
          </section> : viewMode === "structured" ? <section className="structured-review">
          ...
          </section> : <section className="curriculum-review">
            <header className="curriculum-toolbar"><div><strong>人教 A 版教材映射</strong><small>{curriculumBatch ? `${curriculumBatch.mapped_count} 已确认 · ${curriculumBatch.high_confidence_count} 个高置信度 · ${curriculumBatch.review_required_count + curriculumBatch.no_suggestion_count} 个人工复核` : "读取已导入题目"}</small></div><div><button type="button" disabled={busy} onClick={() => loadCurriculumBatch().catch((error: Error) => setMessage(error.message))}>刷新推荐</button><button className="primary" type="button" disabled={busy || !selectedCurriculumIds.length} onClick={() => applyCurriculumMappings(selectedCurriculumIds)}>确认所选 {selectedCurriculumIds.length || ""}</button></div></header>
            <p className="curriculum-safety-note">推荐只用于缩小范围，不会自动写入题库。系统已自动勾选置信度不低于 90% 的专用术语命中项；低置信度题必须逐题确认。</p>
            {!curriculumBatch?.items.length ? <div className="boundary-empty"><span>04</span><strong>先把结构化题目送入私人题库</strong><p>导入后，系统会集中推荐人教 A 版册次、章节和知识点，并保留教师确认门禁。</p></div> : <ResizableColumns className="curriculum-body" storageKey="pdf-curriculum-mapping" initialLeftPercent={42} leftMin={270} rightMin={360} collapse="compact" label="调整教材映射题目列表与确认区宽度"><div className="curriculum-list">{curriculumBatch.items.map((item) => { const suggestion = item.suggestions.find((candidate) => candidate.node_id === curriculumSelections[item.question_id]) ?? item.suggestions[0]; const mapped = item.recommendation_status === "already_mapped"; return <article className={`${activeCurriculumItem?.question_id === item.question_id ? "active" : ""} ${item.recommendation_status}`} key={item.question_id}><label><input type="checkbox" disabled={mapped || !suggestion} checked={selectedCurriculumIds.includes(item.question_id)} onChange={() => toggleCurriculumSelection(item.question_id)} /><span /></label><button type="button" onClick={() => selectCurriculumItem(item)}><strong>{item.stem_plain.replace(/\s+/g, " ").slice(0, 70)}</strong><small>{mapped ? `已映射：${item.current_curriculum.knowledge_point_names.join("、")}` : suggestion ? `推荐：${suggestion.name} · ${Math.round(suggestion.confidence * 100)}%` : "暂无可靠推荐，需人工搜索"}</small></button><em>{mapped ? "已确认" : item.recommendation_status === "high_confidence" ? "高置信" : "需复核"}</em></article>; })}</div>
              {activeCurriculumItem ? <div className="curriculum-editor"><header><div><span>题目来源</span><strong>第 {activeCurriculumItem.source_page_start ?? "—"}{activeCurriculumItem.source_page_end && activeCurriculumItem.source_page_end !== activeCurriculumItem.source_page_start ? `—${activeCurriculumItem.source_page_end}` : ""} 页</strong></div><a href={`/search?q=${encodeURIComponent(activeCurriculumItem.question_id)}`}>进入完整题库审核</a></header><section className="curriculum-question"><span>题干摘要</span><p><MathText text={activeCurriculumItem.stem_plain} /></p></section>{activeCurriculumItem.recommendation_status === "already_mapped" ? <section className="curriculum-current"><span>教师已确认</span><strong>{activeCurriculumItem.current_curriculum.knowledge_point_names.join("、")}</strong><p>{activeCurriculumItem.current_curriculum.volume} · {activeCurriculumItem.current_curriculum.chapter} · {activeCurriculumItem.current_curriculum.section}</p></section> : activeCurriculumItem.suggestions.length ? <section className="curriculum-candidates"><header><strong>知识点候选</strong><small>选择后仍需点击确认，才会写入题库</small></header>{activeCurriculumItem.suggestions.map((suggestion) => <label className={curriculumSelections[activeCurriculumItem.question_id] === suggestion.node_id ? "selected" : ""} key={suggestion.node_id}><input type="radio" name={`curriculum-${activeCurriculumItem.question_id}`} checked={curriculumSelections[activeCurriculumItem.question_id] === suggestion.node_id} onChange={() => setCurriculumSelections((current) => ({ ...current, [activeCurriculumItem.question_id]: suggestion.node_id }))} /><div><strong>{suggestion.name}</strong><span>{suggestion.volume} · {suggestion.chapter} · {suggestion.section}</span><small>{suggestion.reasons.join("；")}</small></div><em>{Math.round(suggestion.confidence * 100)}%</em></label>)}<button type="button" className="curriculum-apply-one" disabled={busy || !curriculumSelections[activeCurriculumItem.question_id]} onClick={() => applyCurriculumMappings([activeCurriculumItem.question_id])}>确认当前题映射</button></section> : <section className="curriculum-no-match"><strong>没有足够可靠的自动建议</strong><p>此题不会进入批量确认。请到完整题库审核中搜索教材目录并人工选择。</p><a href={`/search?q=${encodeURIComponent(activeCurriculumItem.question_id)}`}>前往人工映射</a></section>}</div> : <div />}
            </ResizableColumns>}
          </section>}
        </ResizableColumns>
        <footer className="import-next-stage"><div><span>{viewMode === "structured" || viewMode === "curriculum" ? "题库安全门" : "下一加工环节"}</span><strong>{viewMode === "pages" ? "题目边界与小问校对" : viewMode === "boundaries" ? "题干、选项、公式与配图校对" : viewMode === "structured" ? "批量标注人教 A 版知识点" : "继续独立数学核验与教师审核"}</strong><p>{viewMode === "pages" ? "在页坐标和文字层之上生成拆题候选，并继续保持教师确认门禁。" : viewMode === "boundaries" ? "只有教师确认过的边界才会生成结构化草稿，且不会自动入库。" : viewMode === "structured" ? "已导入题仍是私人待审核草稿，教材映射必须由教师确认。" : "教材映射完成后仍不会公开；还需在题库审核中完成数学核验和内容审核。"}</p></div>{viewMode === "pages" ? <button type="button" disabled={selected.status !== "ready_for_segmentation"} onClick={() => { setViewMode("boundaries"); if (!boundaries.total) proposeBoundaries(); }}>{selected.status === "ready_for_segmentation" ? "进入边界校对" : "请先完成页面分析"}</button> : viewMode === "boundaries" ? <button type="button" disabled={!boundaries.confirmed_count || busy} onClick={() => { setViewMode("structured"); if (!drafts.total) proposeStructuredDrafts(); }}>进入内容结构化</button> : viewMode === "structured" ? <button type="button" disabled={!drafts.imported_count || busy} onClick={() => { setViewMode("curriculum"); loadCurriculumBatch().catch((error: Error) => setMessage(error.message)); }}>进入教材映射</button> : <em>{curriculumBatch?.mapped_count ?? 0}/{curriculumBatch?.total ?? 0} 已映射</em>}</footer>
      </>}</main>
    </ResizableColumns>
  </div>;
}

...
export default function PapersPage() {
  ...
  function newPaper() { setSelected(null); setTitle("高中数学阶段检测"); setDuration(90); setInstructions(defaultInstructions); setDraftItems([]); setDraftWarnings(["保存试卷后会固定题目和图片快照。"]); setActiveTemplateId(""); setAutoTarget(50); setAutoProfile("balanced"); setAutoTypeCounts({ single_choice: 4, open_response: 2 }); setQuery(""); setMessage("已创建空白组卷草稿，请从左侧加入题目。"); }
  ...
  function applyTemplate(templateId: string) {
    ...
    setAutoTarget(template.target_score); setDuration(template.duration_minutes); setAutoProfile(template.difficulty_profile); setAutoTypeCounts(Object.fromEntries(template.sections.map((section) => [section.question_type, section.count]))); setMessage(`已应用“${template.name}”。系统会先检查当前题库供给，再生成可编辑草稿。`);
  }
